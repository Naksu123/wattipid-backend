...
class PasswordResetRepository {
    ...
    public function getRecentResetToken($email, $seconds = 45) {
        $stmt = $this->conn->prepare("SELECT id FROM password_resets WHERE email = ? AND created_at >= DATE_SUB(NOW(), INTERVAL ? SECOND) ORDER BY created_at DESC LIMIT 1");
        $stmt->execute([$email, (int)$seconds]);
        return $stmt->fetchColumn();
    }
}
